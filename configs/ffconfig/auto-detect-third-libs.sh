...
if [ -d "$lame_path" ]; then
    echo "[✅] auto detect lame installed in $lame_path"
    export PATH="$lame_path:$PATH"
else 
    echo "[❌] unable to find lame in system path"
fi

lame --version &> /dev/null && export enable_lame=1
pkg-config --libs lame --silence-errors >/dev/null
